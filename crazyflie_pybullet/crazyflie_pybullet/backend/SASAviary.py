...
from .CTBRWrapper import CTBRWrapper
...
class SASAviary(BaseAviary):
    ...
    def __init__(self,
        ...
        self.MAX_PWM = 65535
        self.MASS_KG = 0.027
        self.MAX_THRUST = 16*self.MASS_KG
        self.verbose = verbose
        
        # Flag to determine whether or not to use built in controller
        self.use_full_state_cmd = use_full_state_cmd
        self.ctbr_wrapper = CTBRWrapper(drone_model, self.G, self.MAX_THRUST)

        self._initalize_cffirmware()
        self._initialize_ctbr_controller()
    ...
    def _initialize_ctbr_controller(self):
        self.ctbr_cmd = np.zeros(4)
    ...
    def _step_rpy_controller(self):
        roll, pitch, yaw, thrust = self.ctbr_cmd
        # print(f"Control: {roll:.2f}, {pitch:.2f}, {yaw:.2f}, {thrust:.2f}")
        rpms = self.ctbr_wrapper.thrust_rpy_ctrl(thrust, 
    ...
    def sendThrustRPYCmd(self, thrust, roll, pitch, yaw):
        self.ctbr_cmd = np.array([roll, pitch, yaw, thrust])
